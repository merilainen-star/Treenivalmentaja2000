...
the step cue on the duration bullet, inside ASCII quotes. The former separate
header guarded against numeric names being parsed as targets; quoting replaces
that guard rather than dropping its purpose. Embedded quotes are neutralized.
The sample JSON now states the action and interval goal in each name. All
18 stages, 1,905 seconds, 2,400 metres and the 265 s/km work target are retained.
The native TXT was generated by the compiled app exporter and compared exactly.

Measured: **920/0/0 JVM tests**, lint **0 errors / 10 warnings**,
debug APK **22,213,157 bytes = 22.213157 MB**. Build completed successfully.
The HTTP export regression checks step cues, numeric instructions, embedded
quotes and the separate distance/pace target on the actual outgoing payload.
It is a local HTTP server, not the Intervals.icu parser or a physical watch.
No device tests were run for this exporter/data-only fix; no emulator was started.
The user's watch still needs re-export and re-sync to verify its final rendering.
...
./gradlew.bat :app:assembleDebug :app:lintDebug :app:testDebugUnitTest --offline
./.scratch/validate_today_run.ps1 --write
python .scratch/report_watch_cues.py
git diff --check
```

Counts sum root JUnit XML attributes; APK MB is byte length / 1,000,000.
The existing nonfatal KSP/AWT diagnostic appeared; no build guard was removed.
See [Suunto test correction and sources](docs/SUUNTO_TEST_2026-09-13.md).
...
| Check | Command | Measured result |
| --- | --- | --- |
| Unit tests | `./gradlew :app:testDebugUnitTest --rerun` (with `app/build/test-results` cleared first) | 741 tests, 0 failures, 0 errors, 0 skipped |
...
| Instrumented | `adb devices -l` | **Not run: no device or emulator attached.** The change is a callback from the countdown and arithmetic over two maps, so nothing here needs one — but the instrumented suite has not been re-run since, and this line says so rather than implying it passed. |

No baseline moved. The finish screen's per-movement list now reads a held movement's clock rather
than its card time, but no screenshot fixture exercises a held movement with banked holds, so every
comparison is byte-identical.

**Byte-identical APK, and that is not a measurement error.** 21,498,867 B both before and after,
