...
class RunExportTest {
  ...
  @Test fun `exports run stages and local date but no strength past or closed sessions`() = runTest {
    ...
    assertTrue(description, description.contains("- 600s"))
    assertTrue(description, description.contains("- 1000mtr 5:00/km Pace"))
    assertTrue(description, description.contains("- 120s"))
