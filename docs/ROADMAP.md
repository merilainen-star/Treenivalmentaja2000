...
1. **Strava integration** — running load into the app: pace, distance, time, calories per run,
   matched to planned running sessions the way Oura workouts already match. OAuth with
   user-entered credentials in Settings, same pattern as Oura
   ([ADR-009](DECISIONS.md#adr-009-the-oura-client-credentials-are-entered-in-the-app-not-compiled-into-it)).
2. **Readiness rule, deterministic (Phase A)** — no AI anywhere in this step. Yesterday's
   readiness was poor and the session was missed (or done anyway): the next morning the app asks
   whether to shift the programme forward and start lighter today. The question is generated by a
   rule and executed by the `TrainingEngine` operations that already exist (move, lighten); this
   is the first point where the readiness number *acts* on the plan instead of only being shown.
