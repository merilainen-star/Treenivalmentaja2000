...
| `data/update/UpdateInfoParsingTest`, `domain/CheckForUpdateUseCaseTest` | The published build metadata, parsed through the real Moshi configuration, and the comparison against the installed version. Including the four payloads that must be refused rather than installed: no digest, a digest that is not a SHA-256, a plain-HTTP download URL and a zero byte count. |
| `data/update/ApkTransferTest` | The integrity check without a `PackageInstaller`: a matching file accepted byte for byte, a truncated one and a same-size-different-content one both rejected, an endless stream stopped rather than written out, and progress that reaches 100 without going backwards. The one step of the update that can silently do the wrong thing. |
| `data/update/InstallFailureMessageTest`, `domain/InstallUpdateUseCaseTest` | Every `PackageInstaller` outcome having a sentence of its own — pressing "Peruuta" arrives as a status code and nothing else — and the mapping to what the card shows, including that a cancelled install offers the same release again instead of sending the user back to check. |
...
| `data/update/UpdateInstalledNotificationTest` (instrumented) | That the "app updated" notice appears for `ACTION_MY_PACKAGE_REPLACED` and for neither of the other two actions `BootReceiver` handles, that it carries a content intent, and that a second update replaces the notice rather than stacking one. Asserted against the system notification manager, with `POST_NOTIFICATIONS` granted by rule. |
| `data/update/UpdateInstallReceiverTest` (instrumented) | That the install callback starts the intent it carries **only** for `STATUS_PENDING_USER_ACTION`, and never for a delivery with another action or with no intent in it. Instrumented because the assertion is an actual activity launch, counted with an `ActivityMonitor`. The receiver is not exported, so this cannot be reached from outside the app — this pins the second guard, for the day someone moves the callback somewhere that can. |
